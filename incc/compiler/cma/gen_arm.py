...
from .arm64_helper import arm64_final, arm64_prefix, l, t
...
def ir_to_arm64(cma_code, env) :
    ...
    for line in cma_code.splitlines() :
        code += c(f";\t\t\t\t\t\t\t\t\t\t{print_em}{line}") #<<< print each lines with info
...
##LABEL ++!
            ...
            case ['loada', addr]:
                code += ir_to_arm64(f'loadc {addr}',env) + ir_to_arm64('load',env) 
            ...
            case [*unknown] :
                code += l(f'Error: unknown CMa statement {unknown}<<<<<<<<<<<<<<<<<<<=========')
...
def arm64_final_prog(arm64_code, env, stack_input_for_print=None) :
    ...
    program += l(f";inside STACK_MACHINE: push(+)/pop(-) count = {COUNTER_PUSH_POP}")
    return program
